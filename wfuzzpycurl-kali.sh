...
#
#   How to rebuild PycURL against OpenSSL on Kali Rolling
#   (tested for Kali Rolling 2018.4)
#   Edit https://github.com/adon90/openssl_wfuzz/wfuuzpycurl.sh specially for Kali
#   By Volken
#
...
sudo apt-get install libcurl4-openssl-dev libssl-dev --yes
...
header "7. delete previous dir"
sudo rm -R pycurl-7.43.0.1

header "8. unpack source archive"
sudo dpkg-source -x $DSC

header "9. change to package dir"
cd $DIR

header "10. edit debian/control file"
...
header "12. install via package manager"
sudo dpkg -i ../python-pycurl_*.deb

header "13. make the change permanent"
sudo apt-mark hold python-pycurl

header "14. reinstalling wfuzz with openssl"
